...
const calcEconomy = (runsConceded, overs) => {
  if (overs <= 0 || runsConceded < 0) return 0;
  const economy = parseFloat((runsConceded / overs).toFixed(2));
  return economy;
};

const calcBattingAvg = (totalRuns, innings, notOuts = 0) => {
  if (innings - notOuts <= 0) return 0;
  const battingAvg = parseFloat((totalRuns / (innings - notOuts)).toFixed(2));
  return battingAvg;
};

const isAllRounder = (battingAvg, economy) => {
  if (battingAvg > 30 && economy < 8) return true;
  else return false;
};

const getPlayerCard = (player) => {
  if (typeof player !== "object" || !player || !player.name) return null;
