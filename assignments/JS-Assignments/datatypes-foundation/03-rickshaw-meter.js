...
function roundFare(amount, decimalPlaces) {
  if (
...
function calculateSurge(baseFare, surgeMultiplier) {
  if (
    baseFare < 0 ||
    surgeMultiplier < 0 ||
...
function findCheapestAndCostliest(...fares) {
  const filteredFare = fares.filter(
    (fare) => typeof fare === "number" && !Number.isNaN(fare)
  );

  if (filteredFare.length === 0) return null;

  const cheapest = Math.min(...filteredFare);
  const costliest = Math.max(...filteredFare);

  return { cheapest, costliest };
}

function getDistanceDifference(from, to) {
