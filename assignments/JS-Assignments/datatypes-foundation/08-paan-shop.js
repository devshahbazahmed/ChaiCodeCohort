 ...
 */
function createPaanOrder(basePaan, customizations) {
  if (
...
function updatePrices(menu, increase) {
  if (typeof menu !== "object" || typeof increase !== "number" || menu === null)
    return {};
  const newArr = Object.entries(menu);
  const newMenu = Object.fromEntries(
    newArr.map(([key, value]) => [key, value + increase])
  );
  return newMenu;
}
