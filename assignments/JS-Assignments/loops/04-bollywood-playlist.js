...
function buildPlaylist(songs, maxDuration) {
  let playList = {
    count: 0,
    totalDuration: 0,
  };
  ...
  for (let i = 0; i < songs.length; i++) {
    if (songs[i] > 0) {
      if (playList.totalDuration + songs[i] > maxDuration) {
        break;
      }
      playList.count++;
      playList.totalDuration += songs[i];
    }
  }
...
console.log(buildPlaylist([240, 180, 300, 200], 600));
