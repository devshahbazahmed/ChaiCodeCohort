...
export function setupParadeDashboard(container) {
  ...
  return {
    ...
    removeContingent(name) {
      const element = document.querySelector(`.contingent[data-name=${name}]`);
    ...
    highlightState(state) {
      let count = 0;
      const allChildren = container.querySelectorAll(`.contingent`);
    ...
    getParadeOrder() {
      return Array.from(container.children).map((el) => el.dataset.name);
    },
