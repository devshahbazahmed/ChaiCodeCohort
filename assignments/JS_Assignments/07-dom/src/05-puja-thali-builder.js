...
export function setupToggleItem(button, thaliElement, itemName) {
  if (!button || !thaliElement || !itemName) return null;
  const handler = () => {
    const existingItem = Array.from(thaliElement.children).find(item => item.textContent === itemName);
...
export function createThaliManager(thaliElement, counterElement) {
  ...
  return {
    ...
    removeItem(name) {
      const existingItem = Array.from(thaliElement.children).find(item => item.textContent === name);
    ...
    getCount() {
      return Array.from(thaliElement.children).length;
    },
    clear() {
