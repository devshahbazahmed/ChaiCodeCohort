...
export function updateChaiPrice(document, chaiType, newPrice) {
  if (newPrice <= 0 || !chaiType || typeof chaiType !== "string" || !Number.isInteger(newPrice)) return false;
  const price = document.getElementById(`price-${chaiType}`);
...
export function getChaiPrice(document, chaiType) {
  let price = document.getElementById(`price-${chaiType}`);
  if (!price) return null;
  const prefix = "₹";
  if (price.textContent.startsWith(prefix)) {
    price.textContent = price.textContent.slice(prefix.length);
  }
  const newPrice = +price.textContent;
  return newPrice;
}
...
export function updateStallName(document, newName) {
  ...
  let oldStallName = stallName.textContent;
...
export function highlightCheapestChai(document) {
  ...
  allPrices.forEach(el => {
    const priceElement = el.textContent.replace("₹", "").trim();
  ...
  allPrices.forEach(el => el.classList.remove("cheapest"));
